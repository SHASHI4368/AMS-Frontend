...
const StaffCalendarComponent = () => {
  ...
  const getAllAppointments = async (Lecturer_mail) => {
    try {
      const url = `http://192.168.1.12.nip.io:8080/db/appointments/${Lecturer_mail}`;
  ...
  const getStudentDetails = async (Reg_number) => {
    try {
      const url = `http://192.168.1.12.nip.io:8080/db/student/details/${Reg_number}`;
  ...
  const getAppointment = async (Id) => {
    try {
      const url = `http://192.168.1.12.nip.io:8080/db/appointment/${Id}`;
      const response = await axios.get(url);
