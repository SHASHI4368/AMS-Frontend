...
const Login = () => {
  ...
  const getRegNumber = async (Email) => {
    try {
      const url = `http://192.168.1.12.nip.io:8080/db/student/regnumber/${Email}`;
  ...
  const handleStdLogin = async (Email, Password) => {
    const getStudentAppointments = async () => {
      try {
        const url = `http://192.168.1.12.nip.io:8080/db/student/appointments/${regNumber}`;
        const response = await axios.get(url);
    ...
    try {
      const url = `http://192.168.1.12.nip.io:8080/db/student/login`;
  ...
  const handleStaffLogin = async (Email, Original_password) => {
    ...
    try {
      const url = `http://192.168.1.12.nip.io:8080/db/staff/login`;
  ...
  const getStaffPassword = async (Email) => {
    try {
      const url = `http://192.168.1.12.nip.io:8080/db/staff/password/${Email}`;
      const response = await axios.get(url);
  ...
  const checkStaffIsThere = async () => {
    try {
      const url = `http://192.168.1.12.nip.io:8080/db/staff/${tempEmail}`;
      const response = await axios.get(url);
      ...
      console.log(err);
      if (tempEmail.includes("ruh.ac.lk")) {
